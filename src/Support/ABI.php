...
class ABI
{
    ...
    private static function GetParameterType(?string $abi_string_type)
    {
        ...
        else if (Utils::stringContains($abi_string_type, 'bytes'))     return VariableType::BytesFixed;

        //dynamic
        else if (Utils::stringContains($abi_string_type, 'tuple'))     return VariableType::Tuple;
        else if (Utils::stringContains($abi_string_type, 'string'))       return VariableType::String;

        //static 
        else if (Utils::stringContains($abi_string_type, 'uint'))        return VariableType::UInt;
        else if (Utils::stringContains($abi_string_type, 'int'))         return VariableType::Int;
        else if (Utils::stringContains($abi_string_type, 'fixed'))       return VariableType::Int;
        else if (Utils::stringContains($abi_string_type, 'bool'))         return VariableType::Bool;
        else if (Utils::stringContains($abi_string_type, 'address'))      return VariableType::Address;
    ...
    private static function EncodeInput($input, $inputData, $round, &$currentDynamicIndex)
    {
        ...
        if ($round == 1) {
            ...
            //dynamic
            if (Utils::stringContains($input->type, '[')) {
                ...
                $isStaticLength = $isStaticArray && !Utils::stringContains($input_type, '[]');
    ...
    public static function DecodeGroup($outputs, $encoded, $index)
    {
        ...
        foreach ($outputs as $output) {
            ...
            //dynamic
            if (Utils::stringContains($output->type, '[')) {
                ...
                $isStaticLength = $isStaticArray && !Utils::stringContains($output->type, '[]');
    ...
    private static function DecodeInput_Array($output, $encoded, $index)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            $res = "error";
            if (Utils::stringContains($clean_output->type, '[')) {
                $isStaticLength = $isStaticType && !Utils::stringContains($clean_output->type, '[]');
                //arrays with all static parameters have no initial array offset 
                $element_start = $index;
                if ($isStaticLength) {
                    $element_start = $index;
    ...
    private static function GetOutputOffset($output): int
    {
        ...
        if (Utils::stringContains($output_type, '[')) {
    ...
    private static function GetOutputOffset_StaticComponents($components): int
    {
        ...
        foreach ($components as $comp) {
            ...
            if (Utils::stringContains($output_type, '[') || $varType == VariableType::Tuple) {
